...
: ${font:=$(convert -list Font | sed -ne '/Font:.*Sans/{s@.*: *@@;p;q}')}

gen_draw_args() {
  local ps=${1:-24} dev_x dev ps_x
  local loc_y="$ps"
  local iface_list="$(ip a show scope global | grep -o '^[0-9]\+:[^:]\+:\|inet6\? [^ ]*' | cut -f2 -d" " | grep -v 127.0.0.1 | cut -f1 -d/)"
...
disp_size() {
  xdpyinfo | sed -ne '/dimensions:/{s/.*[[:space:]]\([0-9]\+x[0-9]\+\).*/\1/;p;q}'
}
...
update_bg() {
  if check_wp_settings;then
    orig_md5="$(echo -n "$orig" | md5sum | cut -c-8)"
    sz="$(disp_size)"
    local orig_cache="${d}/cache-${orig_md5}-${sz}.png"
    test -e "$orig_cache" || convert "$orig" -geometry "$sz" "$orig_cache"
    set_wp "$orig_cache"
    convert "$orig_cache" -fill white -font "$font" -gravity NorthEast $(gen_draw_args $(($(identify -format %h "$orig_cache")/30)))  "$modded" || return
    set_wp "$modded"
  fi
}
